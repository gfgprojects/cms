...
Open the RS perspective. To do that click Window $\rightarrow$ Perspective $\rightarrow$ open perspective $\rightarrow$ other. Choose Repast Simphony in the window which appears.
...
\vskip2mm
The GUI execution is activated by clicking the \verb+cms Model+ item and after few seconds the model window will show up. Click the \includegraphics[scale=0.4]{fig_home} button, and you should see something similar to the next figure in your screen.
...
It make sense to run the model in batch mode if it collects the data of the run. Follow the instructions given in the ``Data Collection for Repast Java and ReLogo'' document provided in the documentation section of the RS web site to create text sinks.   
