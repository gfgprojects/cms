...
\subsubsection{Parametrization}
The best way to control the setup is to set the \verb+autarkyAtTheBeginning+ to true. In this way the prices on the various market session can be controlled by moving the demand curve intercept in the \verb+buyers.csv+ file.
...
deliver two very close initial prices prices in the two market session.

Note that the autarky is preserved by choosing a high level of the \verb+percentageOfPriceMarkDownInNewlyAccessibleMarkets+ parameter.
